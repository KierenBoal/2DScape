...
final class BillboardClassificationDebug
{
	...
	BillboardClassificationDebug(Client client, NpcSnapConfig config, Logger log)
	{
		this.client = client;
		this.config = config;
		this.log = log;
	...
	ClassifiedObjectType resolveObservedTileObjectType(ObservedTileObject observed)
	{
		if (observed == null)
		{
	...
	void logDecision(Object source, ObjectClassifier.ClassificationDecision decision)
	{
		logDecision(source, decision, null);
	}

	void logDecision(Object source, ObjectClassifier.ClassificationDecision decision, String extraTypes)
	{
	...
	String actorDebugKey(Actor actor)
	{
