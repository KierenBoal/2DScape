...
final class BillboardRenderQuality
{
	static final double MIN_RENDER_QUALITY = 0.01d;

	private BillboardRenderQuality()
	{
	}

	static double fromPercent(double qualityPercent)
	{
		return clamp(qualityPercent / 100.0d);
	}

	static double clamp(double qualityScale)
	{
		return Math.max(MIN_RENDER_QUALITY, Math.min(1.0d, qualityScale));
	}

	static int key(double qualityScale)
	{
		return (int) Math.round(clamp(qualityScale) * 10_000.0d);
	}

	static double seededScale(double fullQualityScale, int nearPriorityIndex, int maxUpdatesPerFrame)
	{
