...
final class BillboardColorUtils
{
	...
	static Color decodedLitFaceColor(int face, int[] faceColors1, int[] faceColors2, int[] faceColors3, int alpha)
	{
	...
	static Integer faceColorValue(int[] faceColors, int face)
	{
		return faceColors != null && face < faceColors.length ? faceColors[face] : null;
	}

	static Integer unlitFaceColorValue(short[] unlitFaceColors, int face)
	{
	...
	static Color applyLightBoost(Color color, double lightBoostPercent)
	{
	...
	static int modulateTexturePixel(int samplePixel, Color shade, int colorBands)
	{
